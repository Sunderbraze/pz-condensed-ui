...
local UI_BORDER_SPACING = 9
local FONT_HGT_SMALL = 9
local BUTTON_HGT = FONT_HGT_SMALL + 5
local SCROLL_BAR_WIDTH = 1
...
function ISCharacterInfo:createChildren()
	...
	for i,_ in ipairs(self.sorted) do
		...
		local collapseButton = ISButton:new(UI_BORDER_SPACING+1, 0, 0, 0, "", self, ISCharacterInfo.collapseSection);
...
function ISCharacterInfo:render()
	...
	elseif ISCharacterInfo.timerMultiplierAnim <= 1500 then
        ISCharacterInfo.animOffset = 6;
	elseif ISCharacterInfo.timerMultiplierAnim <= 2000 then
        ISCharacterInfo.animOffset = 12;
	else
		ISCharacterInfo.timerMultiplierAnim = 0;
	end
	...
	for i,parentPerk in ipairs(self.sorted) do
		...
		--self:drawText(parentPerk:getName(), left+BUTTON_HGT+FONT_HGT_SMALL, y+fontOffset, 1, 1, 1, 1, UIFont.Small);
		--self.buttonList[i]:setY(y)
		--y = y + math.max((BUTTON_HGT/2));
		...
		if not self.collapse[i] then
			...
			for ind, perk in ipairs(perkList) do
				...
				self:drawText(perk:getName(), left + UI_BORDER_SPACING, y+fontOffset, r, g, b, 1, UIFont.Small);
				...
				if self.char:getXp():getMultiplier(perk:getType()) > 0 then
					...
					local arrowOffset = math.floor((BUTTON_HGT-arrowHeight)/2)
					self:drawTexture(self.disabledArrow, (self.txtLen*1.3)-UI_BORDER_SPACING+1-arrowPixelOffset, y+arrowOffset, 1, 1, 1, 1);
					self:drawTexture(self.disabledArrow, (self.txtLen*1.3)-UI_BORDER_SPACING+1+6-arrowPixelOffset, y+arrowOffset, 1, 1, 1, 1);
					self:drawTexture(self.disabledArrow, (self.txtLen*1.3)-UI_BORDER_SPACING+1+12-arrowPixelOffset, y+arrowOffset, 1, 1, 1, 1);

					if ISCharacterInfo.animOffset > -1 then
						self:drawTexture(self.arrow, (self.txtLen*1.3)-UI_BORDER_SPACING+1 + ISCharacterInfo.animOffset - arrowPixelOffset, y+arrowOffset, 1, 1, 1, 1);
					end
				end
				if not self.progressBarLoaded then
					...
					local progressBar = ISSkillProgressBar:new(left + 36 + self.txtLen, y+skillPointOffset, 0, 0, self.playerNum, perk, self);
	...
	self:setWidthAndParentWidth(235);
	self:setHeightAndParentHeight(495);
